            ...
            <div class="col-md-2">
                <div class="box border0">
                    <ul class="tablists">
                        ...
                        <?php } if ($this->rbac->hasPrivilege('department', 'can_view')) { ?>
                            <li><a href="<?php echo base_url(); ?>admin/hospitaldepartment/"><?php echo $this->lang->line('hospital') ." ". $this->lang->line('department'); ?></a></li>
                        <?php } ?>  
                        <!-- <?php if ($this->rbac->hasPrivilege('department', 'can_view')) { ?>
                            ...
                            <table class="table table-striped table-bordered table-hover example">
                                ...
                                <tbody>
                                    ...
                                    foreach ($designation as $value) {
                                        ...
                                        ?>
                                        <tr>
                                        <td class="mailbox-name"> <?php echo $value['role_name'] ?></td>
...
//alert(id);
        ...
        $.ajax({
            ...
            success: function (result) {
               // alert(result.designation);
                var roleId = result.role;
                $('#degi_id').val(result.id);
                $('#type1').val(result.designation);
                var roleDropdown = $('#role1');
                roleDropdown.empty();
var emptyOption = $('<option></option>').val('').text('Select');
roleDropdown.append(emptyOption);
<?php foreach ($roles as $role) { ?>
    var option = $('<option></option>')
        .val('<?php echo $role['id']; ?>')
        .text('<?php echo $role['name']; ?>');

    if ('<?php echo $role['id']; ?>' == roleId) {

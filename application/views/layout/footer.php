...
<footer class="main-footer">
    <div class="container">
        <div class="row" >
            ...
            <div class="col-md-6 text-md-left">
                Powered By <!-- <a href="https://3sdsolutions.com/"> --><span style="color:#634297">3SD Solutions & Services Pvt. Ltd<!-- </a> --></span>
            </div>
        </div>
    </div>
</footer>
...
//       });
    ...
    //          var $this = $(this); // Cache the current element(added by yagyaseni)
    //          $this.datetimepicker({
    //              format: datetime_format,
    //              // autoclose: true , // Uncomment if you want to try the autoclose again(not working)
    //              locale: '<?php echo $language_name ?>'
    //          });
            
    //          //added by yagyaseni
    //          $this.on('dp.change', function() {
    //              $this.data("DateTimePicker").hide(); // Manually trigger the close
    //          });
    //      });
    $(document).ready(function (e) {
        var datetime_format = '<?php echo $result = strtr($this->customlib->getSchoolDateFormat(true, true), ['d' => 'DD', 'm' => 'MM', 'Y' => 'YYYY', 'H' => 'hh', 'i' => 'mm',]) ?>';
    ...
    $("body").delegate(".datetimer", "focusin", function () {
            $(this).datetimepicker({
                format: datetime_format,
                locale:
                        '<?php echo $language_name ?>',
     ...
     //    $(document).ready(function (e) {
  //  var datetime_format = '<?php echo strtr($this->customlib->getSchoolDateFormat(true, true), ['d' => 'DD', 'm' => 'MM', 'Y' => 'YYYY', 'H' => 'hh', 'i' => 'mm']); ?>';
...
//             $(this).data("DateTimePicker").hide(); // Hide datepicker after selection
//         });
//     });
// });
...
//             locale: '<?php echo $language_name ?>',
//             maxDate: moment().endOf('day'), // Restricts future dates
//             useCurrent: false // Prevents updating the date to the current time on focus
//         }).on('dp.change', function (e) {
//             // Get the selected date
//             var picker = $(this).data("DateTimePicker");
//             var selectedDate = picker.date();  // Selected date

//             // Get current time
//             var currentMoment = moment();

//             // Update selected date to have the current time (hours, minutes, seconds)
//             selectedDate.set({
...
//             // Update picker with the selected date + current time
//             picker.date(selectedDate);

//             // Close the picker after date is selected
...
var date_format = '<?php echo $result = strtr($this->customlib->getSchoolDateFormat(), ['d' => 'dd', 'm' => 'mm', 'Y' => 'yyyy',]) ?>';
        ...
        $("body").delegate(".datet", "focusin", function () {
 ...
 var date_formatt = '<?php echo $result = strtr($this->customlib->getSchoolDateFormat(), ['d' => 'dd', 'm' => 'mm', 'Y' => 'yyyy',]) ?>';
var capital_date_formatt = date_formatt.toUpperCase();
$.fn.dataTable.moment(capital_date_formatt);

$("body").delegate(".datee", "focusin", function () {
    // Get today's date
    var today = new Date();
    ...
    // Format the max date according to the custom date format
    var maxDate = date_format.replace('dd', day).replace('mm', month).replace('yyyy', year).replace('YYYY', year).replace('DD', day).replace('MM', month);
    ...
    $(this).datepicker({
        ...
        autoclose: true,
        language: '<?php echo $language_name ?>',
        endDate: maxDate // Set end date to today
    });
});

// var daterange_format = '<?php echo $result = strtr($this->customlib->getSchoolDateFormat(), ['d' => 'DD', 'm' => 'MM', 'Y' => 'YYYY']) ?>';

// // Get today's date and calculate the start and end dates for the current year
// var today = new Date();
// var startOfYear = new Date(today.getFullYear(), 0, 1); // January 1st of the current year
// var endOfYear = new Date(today.getFullYear(), 11, 31); // December 31st of the current year

// var startDateFormatted = moment(startOfYear).format(daterange_format);
// var endDateFormatted = moment(endOfYear).format(daterange_format);

// $("body").delegate(".daterangee", "focusin", function () {
//     $(this).daterangepicker({
//         locale: {
//             format: daterange_format,
//         },
       
//         minDate: startDateFormatted, // Set the minimum date to the start of the year
...
var daterange_format = '<?php echo $result = strtr($this->customlib->getSchoolDateFormat(), ['d' => 'DD', 'm' => 'MM', 'Y' => 'YYYY']) ?>';
    ...
    // Get today's date
    var today = moment(); // Using moment to get the current date

    // Set the end of the year
    var endOfYear = moment().endOf('year'); // December 31st of the current year

    // Format today's date and the end of the year
    var todayFormatted = today.format(daterange_format);
    ...
    $("body").delegate(".daterangee", "focusin", function () {
        $(this).daterangepicker({
            ...
            minDate: todayFormatted,  // Set the minimum date to today
            maxDate: endDateFormatted, // Set the maximum date to the end of the year
        });
    });
        var daterange_format = '<?php echo $result = strtr($this->customlib->getSchoolDateFormat(), ['d' => 'DD', 'm' => 'MM', 'Y' => 'YYYY']) ?>';
    ...
    if (isset($opd_data['opd_data']['patient_id'])) {
        ...
        <script type="text/javascript">
           // var datetime_format = '<?php //echo $result = strtr($this->customlib->getSchoolDateFormat(true, true), ['d' => 'dd', 'm' => 'MM', 'Y' => 'yyyy', 'H' => 'hh', 'i' => 'mm',]) ?>';
            ...
            var datetime_format = "<?php echo date($this->customlib->getSchoolDateFormat(true, true), strtotime($opd_data['opd_data']['appointment_date'])); ?>";
    // Populate the form fields with the data
    $("#admission_date").val(datetime_format);
    ...
    <?php } else {
        ...
        <script type="text/javascript">
           // var datetime_format = '<?php //echo $result = strtr($this->customlib->getSchoolDateFormat(true, true), ['d' => 'dd', 'm' => 'MM', 'Y' => 'yyyy', 'H' => 'hh', 'i' => 'mm',]) ?>';
            var opd_data = '<?php echo json_encode($opd_data['opd_data']) ?>';
            var data = JSON.parse(opd_data);
            var countryCode = data.phone.substring(4);
            var datetime_format = "<?php echo date($this->customlib->getSchoolDateFormat(true, true), strtotime($opd_data['opd_data']['appointment_date'])); ?>";
            // console.log(data);
            $("#name").val(data.patient_name);
            $("#addformemail").val(data.email);
            $("#number").val(countryCode);
...
if (isset($ipd_data) && (!empty($ipd_data))) {
    if (isset($ipd_data['ipd_data']['patient_id'])) {
        ...
        <script type="text/javascript">
            ...
            var datetime_format = "<?php echo date($this->customlib->getSchoolDateFormat(true, true), strtotime($ipd_data['ipd_data']['appointment_date'])); ?>";
            $("#admission_date").val(datetime_format);
            $("#admission_date").val(datetime_format);
           // $("#admission_date").val(new Date(data.appointment_date).toString(datetime_format));
            $("#consultant_doctor").val(data.cons_doctor);

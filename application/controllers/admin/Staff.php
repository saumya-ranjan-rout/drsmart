...
class Staff extends Admin_Controller
{
    ...
    /*public function email(){
        $email='snigdha.sahoo@3sdsolutions.com';
        $password='snigdha';
         $teacher_login_detail = array('id' => '5', 'credential_for' => 'staff', 'username' => $email, 'password' => $password, 'email' => $email);
...
*/
    ...
    //     $data["contract_type"] = $this->contract_type;
    //     $data["Staff_id"] = $this->staff_model->getLaststaffid();
    ...
    //         'contactno',
    //         $this->lang->line('phone'), // Correct label for the phone number
    //         array(
    //             'required',
    //             array(
    //                 'valid_phone_id',
    //                 array($this->staff_model, 'valid_phone_id')
    //             )
    //         )
    //     );
    ...
    //         $emergency_no = $countryCode.$number;
    ...
    function create()
    {
        ...
        $data["designation"] = $designation;
        $department = $this->staff_model->getDepartment();
        $data["nurses"] = $this->staff_model->getNurses();
        $data["department"] = $department;
        $marital_status = $this->marital_status;
        $data["marital_status"] = $marital_status;
        $data["bloodgroup"] = $this->blood_group;
        ...
        $this->form_validation->set_rules( 'contactno',  $this->lang->line('phone'), array( 'required', array( 'valid_phone_id', array($this->staff_model, 'valid_phone_id')  ) ));

        $this->form_validation->set_rules(
            'emg_contact', // Updated field name
            $this->lang->line('emergency_contact'), // Correct label for the phone number
            array(
                array(
                    'valid_emg_contact',
                    array($this->staff_model, 'valid_emg_contact') // Separate validation function
                )
            )
        );
        ...
        $this->form_validation->set_rules(
            'employee_id',
            $this->lang->line('staff') . " " . $this->lang->line('id'),
            array(
                'required',
                array('check_exists', array($this->staff_model, 'valid_employee_id'))
            )
...
//     $this->form_validation->set_rules('nurses', $this->lang->line('nurse'), 'trim|required|xss_clean');
// }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $countryCode1 = $this->input->post('countryCode1');
            $number1 = $this->input->post('emg_contact');

            $contact_no = $countryCode . $number;
            $emergency_no = $countryCode1 . $number1;
            $email = $this->input->post("email");
    ...
    function edit($id)
    {
        ...
        $data['staff'] = $staff;
        $data["nurses"] = $this->staff_model->getNursesEdit($staff['nurses']);
        $data["contract_type"] = $this->contract_type;
    ...
    {
        $role_id = $this->input->post('role');  // Get role ID from POST data
        $selected_designation = $this->input->post('selected_designation'); // Get the previously selected designation

        $this->db->select("staff_designation.*");
        $this->db->where("staff_designation.role", $role_id);
        $query = $this->db->get("staff_designation");
        $data = $query->result_array();

        $options = "<option value=''>" . $this->lang->line('select') . "</option>";
        foreach ($data as $designation) {
            $selected = $designation['id'] == $selected_designation ? 'selected' : ''; // Use selected attribute
            $options .= "<option value='" . $designation['id'] . "' $selected>" . $designation['designation'] . "</option>";
        }

        echo json_encode(['options' => $options, 'selected' => $selected_designation]);
    }
 public function getStaffname($role,$deg) {
      
        $staff_id = $this->input->post('staff');  // Get role ID from POST data
        $selected_designation = $this->input->post('selected_staffname'); // Get the previously selected designation
  
        $this->db->select("staff.*");
        $this->db->from("staff");
        $this->db->join("staff_roles", "staff_roles.staff_id = staff.id", "left");
        $this->db->join("roles", "roles.id = staff_roles.role_id", "left");
        $this->db->where("staff.department", $staff_id);
        $this->db->where("staff.designation", $deg);
        ...
        foreach ($data as $designation) {
            $selected = $designation['id'] == $selected_designation ? 'selected' : ''; // Use selected attribute
            $options .= "<option value='" . $designation['id'] . "' $selected>" . $designation['name'].' '.$designation['surname']. "</option>";
        }

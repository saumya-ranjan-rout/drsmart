...
class Birthordeath extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->helper('customfield_helper');
        $patients = $this->patient_model->getFemalePatientListall();
  ...
  // public function deathreport()
    ...
    public function deathreport() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            redirect('admin/birthordeath/deathreport?search_type=' . urlencode($search_type));
        }
    ...
    public function birthreport() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            // Redirect to avoid form resubmission
            redirect('admin/birthordeath/birthreport?search_type=' . urlencode($search_type));
        }
    ...
    public function addDeathdata()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $ref_year = date('Y', strtotime($death_date));
            ...
            $prefix = $this->setting_model->getPrefixSetting('death_record_reference_no');
            if (empty($prefix)) {
                $prefix = "DREF";
            }
            ...
            if (!empty($insert_id)) {
                $refno = $prefix . $ref_year . $insert_id;
            } else {
                $refno = $prefix . $ref_year;
            }
            ...
            if ($insert_id) {
                ...
                $this->birthordeath_model->addDeathdata($arrayRefno);
            }
    ...
    public function addBirthdata()
    {
        ...
        $this->form_validation->set_rules('number', $this->lang->line('phone'), 'trim|required|callback_valid_mobile_number');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $prefix = $this->setting_model->getPrefixSetting('birth_record_reference_no');
            if (empty($prefix)) {
                $prefix = "BREF";
            }
    ...
    public function update_birth()
    {
        ...
        $this->form_validation->set_rules('number2', $this->lang->line('phone'), 'trim|required|callback_valid_mobile_number');

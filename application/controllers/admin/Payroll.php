...
class Payroll extends Admin_Controller {
  ...
  function paymentSuccess() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $picture = '';
            if (isset($_FILES["cheque_document"]) && !empty($_FILES['cheque_document']['name'])) {


                $fileInfo = pathinfo($_FILES["cheque_document"]["name"]);
                $picture = rand() . '_payroll.' . $fileInfo['extension'];
                move_uploaded_file($_FILES["cheque_document"]["tmp_name"], "./uploads/payment_document/" . $picture);
            }

            $data = array('payment_mode' => $payment_mode, 'cheque_no' => $this->input->post('cheque_no'),
            'cheque_date' => date('Y-m-d H:i:s', $this->customlib->datetostrtotime($this->input->post('cheque_date'))),
            'document' => $picture, 'payment_date' => $payment_date, 'remark' => $remark, 'status' => $status);
    ...
    public function payrollreport() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            // Capture inputs from the POST request
            $month = $this->input->post("month");
            $year = $this->input->post("year");
            $role = $this->input->post("role");
    
            // Redirect to avoid form resubmission
            redirect('admin/payroll/payrollreport?month=' . $month . '&year=' . $year . '&role=' . $role);
        }
        ...
        if (empty($month)) {
            $month = ""; // Set default value if needed
        }
        if (empty($year)) {
            $year = ""; // Set default value if needed
        }
        if (empty($role)) {
            $role = ""; // Set default value if needed
        }
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // Generate the report if validation is successful
            $result = $this->payroll_model->getpayrollReport($month, $year, $role);
            $data["result"] = $result;
            $this->load->view("layout/header", $data);
            $this->load->view("admin/payroll/payrollreport", $data);
   ...
   public function payrollsearch() {
    ...
    // Check if the request method is POST
    if ($this->input->server('REQUEST_METHOD') == 'POST') {
        // Capture search type and redirect with query parameters
        $search_type = $this->input->post("search_type");
        
        // Redirect to avoid form resubmission
        redirect('admin/payroll/payrollsearch?search_type=' . $search_type);
    }

    // Handle GET request
    $search_type = $this->input->get("search_type") ? $this->input->get("search_type") : "this_month";

    // Fetch result list based on search type

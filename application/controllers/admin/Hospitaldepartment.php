...
class Hospitaldepartment extends Admin_Controller {
    ...
    function index() {
        ...
        $this->form_validation->set_rules(
            ...
            array('check_exists', array($this->Hospitaldepartment_model, 'valid_department'))
                )
        );

        $data["title"] = "Add Hospital Department";
        if ($this->form_validation->run()) {
            ...
            $insert_id = $this->Hospitaldepartment_model->addDepartmentType($data);
            $this->session->set_flashdata('msg', '<div class="alert alert-success">Record added Successfully</div>');
            redirect("admin/department");
        } else {

            $this->load->view("layout/header");
            $this->load->view("admin/staff/hospital_department", $data);
    ...
    function get() { //get product data and encode to be JSON object
        ...
        echo $this->Hospitaldepartment_model->getall();
    }

    function get_data($id) {
        $result = $this->Hospitaldepartment_model->getDepartmentType($id);
    ...
    function edit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $type = $this->input->post("type");
            $data = array('department_name' => $type, 'is_active' => 'yes', 'id' => $departmenttypeid);
            $this->Hospitaldepartment_model->addDepartmentType($data);
    ...
    function departmentdelete($id) {
        ...
        if (!empty($id)) {
            $this->Hospitaldepartment_model->deleteDepartment($id);
        }
        redirect("admin/department");
    }

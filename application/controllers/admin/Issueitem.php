...
class Issueitem extends Admin_Controller {
    ...
    function add() {
        ...
        $this->form_validation->set_rules('return_date', $this->lang->line('return_date'), 'callback_check_return_date');
    ...
    public function check_return_date()
    {
        $issue_date = strtotime($this->input->post('issue_date'));
        $return_date = strtotime($this->input->post('return_date'));

        // the below commented condition is for future date
        // if ($return_date && $return_date < $issue_date) {
        if ($return_date && $return_date <= $issue_date) {
            $this->form_validation->set_message('check_return_date', $this->lang->line('return_date_must_be_greater_than_issue_date'));
            return FALSE;
        }
        return TRUE;
    }
   ...
   public function check_available_quantity()
    {
        ...
        if ($quantity != "" && $item_category_id != "" && $item_id != "") {
            ...
            if ($quantity <= 0) {
                $this->form_validation->set_message('check_available_quantity', $this->lang->line('quantity_must_be_greater_than_zero'));
            } else {
                $this->form_validation->set_message('check_available_quantity', $this->lang->line('quantity_is_greater_than_available_quantity') . " " . $available);
            }
            return FALSE;
        }
        return TRUE;
...
public function issueinventoryreport() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            redirect('admin/issueitem/issueinventoryreport?search_type=' . $search_type);
        }

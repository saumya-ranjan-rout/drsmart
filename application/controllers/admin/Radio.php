...
class Radio extends Admin_Controller
{
    ...
    public function search()
    {
        ...
        $data['charge_category'] = $this->radio_model->getChargeCategoryRadio();
    ...
    public function testReportBatch()
    {
        ...
        $this->form_validation->set_rules('radiology_report', $this->lang->line('file'), 'callback_handle_upload');
        ...
        if ($this->form_validation->run() == FALSE) {
            $msg = array(
                ...
                'radiology_report' => form_error('radiology_report'),
            ...
            $prefix = $this->setting_model->getPrefixSetting('radiology_bill');
    ...
    public function getTestReportBatch()
    {
        ...
        if ($role_id == 3) {
            $user_id = $userdata['id'];
            $result = $this->radio_model->getTestReportBatch($id, $role_id, $user_id);
        } else {
 ...
 'reporting_date' => form_error('reporting_date'),
    ...
    public function radiologyReport()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            redirect('admin/radio/radiologyreport?' . $query_string);
        }

...
class Admin extends Admin_Controller {
    ...
    function transfer_manually($id) {
        ...
        $data['id'] = $id;
        $this->load->view('layout/header', $data);
        $this->load->view('admin/transfer_manually', $data);
        $this->load->view('layout/footer', $data);
    }
    public function patient_company_details()
    {
    ...
    public function add_manually_transfer()
    {
        // if (!$this->rbac->hasPrivilege('pathology test', 'can_add')) {
        //     access_denied();
        // }
        $this->form_validation->set_rules('patientid', $this->lang->line('patient'), 'required');
        $this->form_validation->set_rules('external_hospital', "External Hospital", 'required');
        $this->form_validation->set_rules('external_hospital_address', "External Hospital Address", 'required');
        $this->form_validation->set_rules('external_hospital_phone', "External Hospital Phone", 'required');
        $this->form_validation->set_rules('external_hospital_email', "External Hospital Email", 'required');
        $this->form_validation->set_rules('reason_for_transfer', "Reason For Transfer", 'required');
        $this->form_validation->set_rules('current_diagnosis', "Current Diagnosis", 'required');
        $this->form_validation->set_rules('required_treatment', "Required Treatment", 'required');
        $this->form_validation->set_rules('medical_history', "Medical History", 'required');
        $this->form_validation->set_rules('unavailability', "Unavailable Equipment/Test", 'required');
        $this->form_validation->set_rules('unavailability_reason', "Reason", 'required');
        $this->form_validation->set_rules('consent_for_data_transfer', "Consent For Data Transfer", 'required');
        $this->form_validation->set_rules('representative_name', "Representative Name", 'required');
        $this->form_validation->set_rules('consent_date', "Date", 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            $msg = array(
                ...
                'external_hospital' => form_error('external_hospital'),
                'external_hospital_address' => form_error('external_hospital_address'),
                'external_hospital_phone' => form_error('external_hospital_phone'),
                'external_hospital_email' => form_error('external_hospital_email'),
                'reason_for_transfer' => form_error('reason_for_transfer'),
                'current_diagnosis' => form_error('current_diagnosis'),
                'required_treatment' => form_error('required_treatment'),
                'medical_history' => form_error('medical_history'),
                'unavailability' => form_error('unavailability'),
                'unavailability_reason' => form_error('unavailability_reason'),
                'consent_for_data_transfer' => form_error('consent_for_data_transfer'),
                'representative_name' => form_error('representative_name'),
                'consent_date' => form_error('consent_date'),
            );
            ...
            $report_batch = array( 
                'patientid' => $this->input->post('patientid'),  // Getting patient ID from the form
                'external_hospital' => $this->input->post('external_hospital'),  // External hospital name
                'external_hospital_address' => $this->input->post('external_hospital_address'),  // External hospital address
                'external_hospital_phone' => $this->input->post('external_hospital_phone'),  // External hospital phone
                'external_hospital_email' => $this->input->post('external_hospital_email'),  // External hospital email
                'reason_for_transfer' => $this->input->post('reason_for_transfer'),  // Reason for transfer
                'current_diagnosis' => $this->input->post('current_diagnosis'),  // Current diagnosis
                'required_treatment' => $this->input->post('required_treatment'),  // Required treatment
                'medical_history' => $this->input->post('medical_history'),  // Medical history
                'unavailability' => $this->input->post('unavailability'),  // Unavailability of treatment/equipment
                'unavailability_reason' => $this->input->post('unavailability_reason'),  // Reason for unavailability
                'consent_for_data_transfer' => $this->input->post('consent_for_data_transfer'),  // Consent for data transfer
                'representative_name' => $this->input->post('representative_name'),  // Representative's name
                'consent_date' => date('Y-m-d', strtotime($this->input->post('consent_date'))),  // Consent date formatted as 'Y-m-d'
                'specific_instructions' => $this->input->post('specific_instructions'),  // Representative's name
                'status' => 1,  // Representative's name
                'generated_by' => $this->session->userdata('hospitaladmin')['id'],  // The user generating the report
            );


 $this->db->where('patientid', $report_batch['patientid']);
$query = $this->db->get('transfer_manually');

if ($query->num_rows() > 0) {
    // Update the existing record
    $this->db->where('patientid', $report_batch['patientid']);
    $this->db->update('transfer_manually', $report_batch);
    $insert_id = $query->row()->id; // Assuming you want to retrieve the id of the updated record
    // You can return or use $updated_id as needed
} else {
    // Insert new record
    $this->db->insert('transfer_manually', $report_batch);
    $insert_id = $this->db->insert_id(); // Get the ID of the inserted record
    // You can return or use $insert_id as needed
}
 ...
 $array = array('status' => 'success', 'id' => $insert_id, 'error' => '', 'message' => $this->lang->line('success_message'));
    ...
    public function printTransferManually($id)
    {
        if (!$this->rbac->hasPrivilege('pathology test', 'can_view')) {
            access_denied();
        }

        $print_details = $this->printing_model->get('', 'manually_transfer');
        ...
        $data['result'] = $this->admin_model->getManuallyTransferingPatient($id);
        $data['patientlist'] = $this->patient_model->patientDetails($id);
        $data['settinglist'] = $this->setting_model->get();
       
        $this->load->view('admin/printtransfermanually', $data);
    }
 ...
 public function dashboard() {
    ...
    $medicine = $this->admin_model->count_medicine();
$data["medicine"] = $medicine;
$resultlist = $this->pharmacy_model->search_datatable();
$out_of_stock =0;
$low_stock_count =0;
// print_r($resultlist);
// exit();
...
$data['resultlist'] = $this->appointment_model->searchFullTextDashboard();
   // $low_stock_count = $this->admin_model->count_low_stock();
//$data["low_stock_count"] = $low_stock_count;

 $data['task_details'] = $this->appointment_model->get_task_details($staffid);

$show_popup = $this->session->userdata('show_popup');
    
    // Clear the flag after reading
    $this->session->unset_userdata('show_popup');
    
    // Pass the flag to the view
    $data['show_popup'] = $show_popup;

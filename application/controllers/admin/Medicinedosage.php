...
class Medicinedosage extends Admin_Controller
{
    ...
    public function interval()
    {
        ...
        $this->session->set_userdata('top_menu', 'setup');
        $this->session->set_userdata('sub_menu', 'medicine/index');
        $this->session->set_userdata('sub_sidebar_menu', 'admin/interval');

        $data["title"] = "Add Dosage Interval";
        $medicineDosageInterval = $this->medicine_dosage_model->getMedicineDosageInterval();
        $data["medicineDosageInterval"] = $medicineDosageInterval;


        $this->form_validation->set_rules('interval', $this->lang->line('interval'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run()) {
            ...
            if (!empty($medicinedosageid)) {
                $data = array('interval' => $this->input->post('interval'), 'id' => $medicinedosageid);
            } else {

                $data = array('interval' => $this->input->post('interval'));
            }

            $insert_id = $this->medicine_dosage_model->addMedicineDosage($data);
            $this->session->set_flashdata('msg', '<div class="alert alert-success">Record added Successfully</div>');
            redirect("admin/interval");
        } else {
            $this->load->view("layout/header");
            $this->load->view("admin/pharmacy/medicine_dose_interval", $data);
            $this->load->view("layout/footer");
        }
    }
    ...
    public function addinterval()
    {
        ...
        // Set validation rules for interval only
        $this->form_validation->set_rules('interval', $this->lang->line('interval') . " " . $this->lang->line('name'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $interval = $this->input->post("interval"); // Get interval value

            if (!empty($medicinedosageid)) {
                // Update existing record (without dosage)
                $data = array(
                    'id' => $this->input->post("dosageid"),
                    'dose_interval' => $interval, // Include interval
                );
                $this->medicine_dosage_model->updateDosageInterval($data); // Ensure this method exists
                $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('update_message'));
            } else {
                // Insert new record (without dosage)
                $data = array(
                    'dose_interval' => $interval, // Include interval
                );
                $insert_id = $this->medicine_dosage_model->addDosageInterval($data);
                $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
            }
        }
    ...
    public function get_data_interval($id)
    {
        if (!$this->rbac->hasPrivilege('medicine_category', 'can_view')) {
            access_denied();
        }
        $result = $this->medicine_dosage_model->getMedicineDosageInterval($id);
        echo json_encode($result);
    }
    ...
    public function delete_interval($id)
    {
        ...
        if (!empty($id)) {
            $this->medicine_dosage_model->deleteinterval($id);
    ...
    public function duration()
    {
        ...
        $this->session->set_userdata('top_menu', 'setup');
        $this->session->set_userdata('sub_menu', 'medicine/index');
        $this->session->set_userdata('sub_sidebar_menu', 'admin/duration');

        $data["title"] = "Add Dosage Duration";
        $medicineDosageDuration = $this->medicine_dosage_model->getMedicineDosageDuration();
        $data["medicineDosageDuration"] = $medicineDosageDuration;
        ...
        if ($this->form_validation->run()) {
            ...
            if (!empty($medicinedosageid)) {
                $data = array('duration' => $this->input->post('duration'), 'id' => $medicinedosageid);
            } else {
            ...
            $this->load->view("layout/header");
            $this->load->view("admin/pharmacy/medicine_dose_duration", $data);
            $this->load->view("layout/footer");
        }
    }
    ...
    public function addduration()
    {
        ...
        // Set validation rules for duration only
        $this->form_validation->set_rules('duration', $this->lang->line('duration') . " " . $this->lang->line('name'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // Prepare data for insertion/updating
            $duration = $this->input->post("duration"); // Get duration value

            if (!empty($medicinedosageid)) {
                // Update existing record (without dosage)
                $data = array(
                    'id' => $this->input->post("dosageid"),
                    'dose_duration' => $duration, // Include duration
                );
                $this->medicine_dosage_model->updateDosageDuration($data); // Ensure this method exists
                $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('update_message'));
            } else {
                // Insert new record (without dosage)
                $data = array(
                ...
                $insert_id = $this->medicine_dosage_model->addDosageDuration($data);
                $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
            }
        }
    ...
    public function get_data_duration($id)
    {
        if (!$this->rbac->hasPrivilege('medicine_category', 'can_view')) {
            access_denied();
        }
        $result = $this->medicine_dosage_model->getMedicineDosageDuration($id);
        echo json_encode($result);
    }
    ...
    public function delete_duration($id)
    {
        ...
        if (!empty($id)) {
            $this->medicine_dosage_model->deleteduration($id);
            $array = array('status' => 'success', 'error' => '', 'message' => 'Record deleted Successfully');
        } else {
            $array = array('status' => 'fail', 'error' => '', 'message' => '');
        }

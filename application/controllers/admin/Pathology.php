 ...
 'reporting_date' => form_error('reporting_date'),
    ...
    public function testReportBatch()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $prefix = $this->setting_model->getPrefixSetting('pathology_bill');
    ...
    public function getTestReportBatch()
    {
        ...
        if ($role_id == 3) {
            $user_id = $userdata['id'];
            $result = $this->pathology_model->getTestReportBatch($id, $role_id, $user_id);
        } else {
  ...
  // public function pathologyReport()
    ...
    //         'JOIN patients ON patients.id = pathology_report.patient_id'
    //     );
    ...
    public function pathologyReport()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            redirect('admin/pathology/pathologyreport?' . $query_string);
        }

...
class Expmedicine extends Admin_Controller {
    ...
    public function expmedicinereport08_08() {
    ...
    // $where = "STR_TO_DATE(CONCAT('01-', medicine_batch_details.expiry_date), '%d-%b/%Y') < STR_TO_DATE(CONCAT('01-', '$current_month_year'), '%d-%M/%Y')";
    ...
    //         $search_type = "";
    //         $resultlist = $this->report_model->getExpiryReport($select, $join, $table_name, $where);
    //     } else {

    //         $search_table = "medicine_batch_details";
    //         $search_column = "expiry_date";
    //         $resultlist = $this->report_model->searchExpiryReport($select, $join, $table_name, $search_type, $search_table, $search_column);
    //     }
    ...
    public function expmedicinereport()
{
    ...
    if ($this->input->server('REQUEST_METHOD') === 'POST') {
        ...
        redirect('admin/expmedicine/expmedicinereport?' . $query_string);
    }
    ...
    // Prepare SQL Select and Join clauses
    $select = 'medicine_batch_details.*, pharmacy.medicine_name, pharmacy.medicine_company, pharmacy.supplier, pharmacy.medicine_group, medicine_category.medicine_category';
    $join = array(
    ...
    // Get the current month and year for the expiry date comparison
    $current_month_year = date('M/Y');  // e.g., May/2024

    // Create the WHERE clause for expired medicines
    $where = "STR_TO_DATE(CONCAT('01-', medicine_batch_details.expiry_date), '%d-%b/%Y') < STR_TO_DATE(CONCAT('01-', '$current_month_year'), '%d-%M/%Y')";

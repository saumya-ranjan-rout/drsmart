...
Class Tpa extends Admin_Controller {
    ...
    function master($id) {
      ...
      // Initialize arrays to handle keys without spaces
        $data['org'] = array();
        $data['charge_type_no_spaces'] = array();

        foreach ($data["charge_type"] as $key => $value) {
            // Remove spaces from the key
            $key_no_spaces = str_replace(' ', '_', $key);

            // Store the no-space key in a separate array
            $data['charge_type_no_spaces'][$key_no_spaces] = $value;

            // Fetch values using keys with spaces
            $data['org'][$key_no_spaces] = $this->tpa_model->org_charge($id, $key);
        }

...
class Designation extends Admin_Controller {
    ...
    public function add() {
        ...
        $this->form_validation->set_rules( 'role', $this->lang->line('role'), array('required', array('check_exists', array($this->designation_model, 'valid_designation')) ) );
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array('role' => $role,'designation' => $type, 'is_active' => 'yes');
            $insert_id = $this->designation_model->addDesignation($data);
    ...
    function edit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $type = $this->input->post("type");
            $role = $this->input->post("role");
            $data = array('role' => $role,'designation' => $type, 'is_active' => 'yes', 'id' => $id);

            ...
            if (($opd_ipd_patient_type != 'opd') && ($opd_ipd_patient_type != 'ipd')) {
                ...
                if ($patient_info) {
                    ...
                    $operation_detail = array(
                        ...
                        'charge_id' => $this->input->post('charge_category_id'),
                    ...
                    if (isset($_FILES["document"]) && !empty($_FILES['document']['name'])) {
                        ...
                        $this->operationtheatre_model->operation_detail($data_img);
                    }
                }
            } else {
    ...
    public function otNotification($patient_id = '', $doctor_id, $patientname, $url)
    {
        ...
        if (!empty($patient_id)) {
            $notification_data = array(
                'notification_title' => 'Operation Theatre Visit Created',
   ...
   public function getOperationDetails()
    {
        ...
        $result = $this->operationtheatre_model->getOperationDetails($pid,$id);
        if (($result['patient_type'] == 'Inpatient') || ($result['patient_type'] == 'Outpatient')) {
            $opd_ipd_no = $this->operationtheatre_model->getopdipdDetails($pid, $result['patient_type']);
            $result['opd_ipd_no'] = $opd_ipd_no;
        }
        $result['admission_date'] = date($this->customlib->getSchoolDateFormat(true, true), strtotime($result['admission_date']));
        $result['date'] = date($this->customlib->getSchoolDateFormat(true, false), strtotime($result['date']));
      ...
      public function getOtPatientDetails()
    {
        ...
        $id = $this->input->post("id");
        $result = $this->operationtheatre_model->getOperationPatientDetails($id);
        $result['admission_date'] = date($this->customlib->getSchoolDateFormat(true, true), strtotime($result['admission_date']));
        $result['date'] = date($this->customlib->getSchoolDateFormat(true, false), strtotime($result['date']));
   ...
   public function update()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (isset($_FILES["document"]) && !empty($_FILES['document']['name'])) {
                $fileInfo = pathinfo($_FILES["document"]["name"]);
                $img_name = $otid . '.' . $fileInfo['extension'];
                move_uploaded_file($_FILES["document"]["tmp_name"], "./uploads/payment_document/" . $img_name);
                $data_img = array('id' => $otid, 'payment_doc' => 'uploads/payment_document/' . $img_name);
                $this->operationtheatre_model->update_operation_detail($data_img);
            } else {

                $data_img = array('id' => $otid, 'payment_doc' => '');
                $this->operationtheatre_model->update_operation_detail($data_img);
            }
            $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
        }
 ...
 // public function OtReport()
    ...
    public function OtReport()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            redirect('admin/operationtheatre/otreport?' . $query_string);
        }
        ...
        $where = array("patients.is_active = 'yes' ", "operation_theatre.patient_id = patients.id ");
    
        // Modify where clause based on the selected doctor
        if (!empty($doctorid)) {
            $where[] = "operation_theatre.consultant_doctor =" . (int)$doctorid; // Use intval to prevent SQL injection
        }
    
        // Determine if doctor restriction is enabled
        $userdata = $this->customlib->getUserData();
        $role_id = $userdata['role_id'];
        $doctor_restriction = $this->session->userdata['hospitaladmin']['doctor_restriction'];
        ...
        if (empty($search_type)) {
            $resultlist = $this->report_model->getReport($select, $join, 'operation_theatre', $where);
        } else {
            $search_table = "operation_theatre";
            $search_column = "date";
            $resultlist = $this->report_model->searchReport($select, $join, 'operation_theatre', $search_type, $search_table, $search_column, $where);
        }
    
        // Prepare data for the view
        $data["searchlist"] = $this->search_type; // Assuming $this->search_type is defined elsewhere
        $data["search_type"] = $search_type;
        $data["resultlist"] = $resultlist;
    ...
    public function index()
    {
        ...
        $this->session->set_userdata('top_menu', 'operation_theatre');
        $otCategory = $this->operationtheatre_model->getOtCategory();
        $data["otCategory"] = $otCategory;

        $this->load->view('layout/header');
        $this->load->view('admin/operationtheatre/otCategory.php', $data);
        $this->load->view('layout/footer');
    }
    ...
    public function categoryadd()
    {
        ...
        $this->session->set_userdata('top_menu', 'operation_theatre');
        $this->session->set_userdata('sub_menu', 'admin/operationtheatre/index');
        $otcategoryid = $this->input->post("otcategoryid");


        $this->form_validation->set_rules('category_name', $this->lang->line('operations') . " " . $this->lang->line('category'), 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $insert_id = $this->operationtheatre_model->addOtCategory($data);
        }
    ...
    public function get_data($id)
    {
        ...
        $result = $this->operationtheatre_model->getotCategoryName($id);
        echo json_encode($result);
    }



    public function delete_otcategory($id)
    {
        if (!$this->rbac->hasPrivilege('ot_patient', 'can_delete')) {
            access_denied();
        }
        if (!empty($id)) {
            $this->operationtheatre_model->deleteotcategory($id);
            $array = array('status' => 'success', 'error' => '', 'message' => 'Record deleted Successfully');
        } else {
            $array = array('status' => 'fail', 'error' => '', 'message' => '');
        }
    ...
    public function operationlist()
    {
        ...
        $this->session->set_userdata('top_menu', 'operation_theatre');
        $otList = $this->operationtheatre_model->getOtList();
        $data["otList"] = $otList;
        $data["categories"]  = $this->operationtheatre_model->getCategories();



        $this->load->view('layout/header');
        $this->load->view('admin/operationtheatre/otList.php', $data);
        $this->load->view('layout/footer');
    }
    ...
    public function operationadd()
    {
        ...
        $otlistid = $this->input->post("otlistid");

        $this->form_validation->set_rules('operation_name', $this->lang->line('operations') . " " . $this->lang->line('name'), 'required');
        $this->form_validation->set_rules('category', $this->lang->line('category') . " " . $this->lang->line('name'), 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $insert_id = $this->operationtheatre_model->addOperationList($data);
        }

        echo json_encode($array);
    }

    public function get_data_otlist($id)
    {
        if (!$this->rbac->hasPrivilege('ot_patient', 'can_view')) {
            access_denied();
        }
        $result = $this->operationtheatre_model->getotListName($id);
        echo json_encode($result);
    }


    public function delete_otlist($id)
    {
        if (!$this->rbac->hasPrivilege('ot_patient', 'can_delete')) {
            access_denied();
        }
        if (!empty($id)) {
            $this->operationtheatre_model->deleteotlist($id);
            $array = array('status' => 'success', 'error' => '', 'message' => 'Record deleted Successfully');
        } else {
            $array = array('status' => 'fail', 'error' => '', 'message' => '');
        }

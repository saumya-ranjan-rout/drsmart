...
class Leaverequest extends Admin_Controller {
    ...
    function countLeave($id) {
        ...
        $i = 0;
        $html = "<select name='leave_type' id='leave_type' class='form-control' onChange='GetLbalance(this)'><option value=''>" . $this->lang->line('select') . "</option>";
        $data = array();
            ...
            foreach ($data as $dkey => $dvalue) {
                if (!empty($dvalue["alloted_leave"])) {
                    ...
                    if ($available > 0) {
                        ...
                        $html .= "<option value=" . $dvalue["id"] . " $a data-available='" . $available . "'>" . $dvalue["type"] . " (" . $available . ")" . "</option>";
                    }
                }
            }
        }
    ...
    function countLeavee($id) {
        $lid = $this->input->post("lid");
        $alloted_leavetype = $this->leaverequest_model->allotedLeaveType($id);

        $i = 0;
        $response["html"] = "<select name='leave_type' id='leave_type' class='form-control' onChange='GetLbalance(this)'><option value=''>" . $this->lang->line('select') . "</option>";

        $response["pending_leaves"] = $this->leaverequest_model->countPendingLeaves($id);
        ...
        if (!empty($alloted_leavetype[0]["alloted_leave"])) {
            foreach ($alloted_leavetype as $key => $value) {
                $count_leaves[] = $this->leaverequest_model->countLeavesDataa($id, $value["leave_type_id"]);
                $data[$i]['type'] = $value["type"];
                $data[$i]['id'] = $value["leave_type_id"];
                $data[$i]['alloted_leave'] = $value["alloted_leave"];
                $data[$i]['approve_leave'] = $count_leaves[$i]['approve_leave'];
            ...
            foreach ($data as $dkey => $dvalue) {
                if (!empty($dvalue["alloted_leave"])) {
                    ...
                    if ($available > 0) {
                        ...
                        $response["html"] .= "<option value=" . $dvalue["id"] . " $a data-available='" . $available . "'>" . $dvalue["type"] . " (" . $available . ")" . "</option>";
                    }
                }
            }
        }
    ...
    function leaveStatus($staff_id,$leave_type_id,$days_count,$frmdt,$todt) {
        if ((!$this->rbac->hasPrivilege('approve_leave_request', 'can_edit'))) {
            access_denied();
        }
        ...
        $query = $this->db->select('staff_leave_request.*')  // Select all columns from the table
                ->where('staff_id', $staff_id)     // Filter by staff_id
                ->where('status', 'approve')        // Filter by status
                ->group_start()
                    ->where('leave_from <=', $todt) // Filter by leave_from date
                    ->where('leave_to >=', $frmdt) // Filter by leave_to date
                ->group_end()
     ...
     //echo count($query).'----';
        if ($query && $status=='approve') {
            $array = array('status' => 'fail', 'error' => '', 'message' => 'You can not take leave for same days existing leave');
        } else {
           if ($this->leaverequest_model->changeLeaveStatus($data, $leave_request_id, $staff_id, $leave_type_id, $days_count)) {
                $array = array('status' => 'success', 'error' => '', 'message' => $this->lang->line('success_message'));
               } else {
                $array = array('status' => 'fail', 'error' => 'Insufficient leave balance', 'message' => 'Insufficient leave balance');
              }
        }      

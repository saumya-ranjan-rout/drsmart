...
class Expense extends Admin_Controller {
    ...
    if ($this->input->server('REQUEST_METHOD') == 'POST') {
        ...
        // Redirect to avoid form resubmission and include search_type in URL
        redirect('admin/expense/expensesearch?search_type=' . urlencode($search_type));
    }
    ...
    public function expensegroup() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            // Redirect to avoid form resubmission and include search_type and head in URL
            redirect('admin/expense/expensegroup?search_type=' . urlencode($search_type) . '&head=' . urlencode($head_id));
        }
        ...
        $head_id = $this->input->get('head');
        if (empty($head_id)) {
            $head_id = ''; // Default value if head is not set
        }
    
        // Fetch results based on search type and head ID
        $result = $this->expense_model->searchexpensegroup($search_type, $head_id);

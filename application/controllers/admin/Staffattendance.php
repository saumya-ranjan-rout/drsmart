...
class Staffattendance extends Admin_Controller {
    ...
    public function attendancereport() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            // Capture form data and redirect with query parameters
            $month = $this->input->post('month');
            $year = $this->input->post('year');
            $role = $this->input->post('role');
            
            // Redirect to avoid form resubmission on back/refresh
            redirect('admin/Staffattendance/attendancereport?month=' . $month . '&year=' . $year . '&role=' . $role);
        }
    
        // Handle GET request
        $month = $this->input->get('month') ? $this->input->get('month') : '';
        $searchyear = $this->input->get('year') ? $this->input->get('year') : '';
        ...
        if ($month && $searchyear) {
            ...
            foreach ($res as $result_v) {
                $newdate = date('Y-m-d', strtotime($searchyear . "-" . $month));

...
'code' => form_error('code'),
    ...
    public function update_opdcharges()
    {
        $this->form_validation->set_rules('ecode', $this->lang->line('code'), 'required');
        $this->form_validation->set_rules('echarge_type', $this->lang->line('charge') . " " . $this->lang->line('type'), 'required');
        $this->form_validation->set_rules('echarge_category', $this->lang->line('charge') . " " . $this->lang->line('category'), 'required');
        $this->form_validation->set_rules('eapply_charge', $this->lang->line('applied') . " " . $this->lang->line('charge'), 'required');
        $this->form_validation->set_rules('edate', $this->lang->line('date'), 'required');
        if ($this->form_validation->run() == false) {
            $msg = array(
                'edate' => form_error('edate'),
                'echarge_type' => form_error('echarge_type'),
                'echarge_category' => form_error('echarge_category'),
                'ecode' => form_error('ecode'),
                'eapply_charge' => form_error('eapply_charge'),
            );
            $json_array = array('status' => 'fail', 'error' => $msg, 'message' => '');
        } else {
            $data = array(
                'id' => $this->input->post('uid'),
                'date' => date("Y-m-d", $this->customlib->datetostrtotime($this->input->post('edate'))),
                'charge_id' => $this->input->post('echarge_id'),
                'org_charge_id' => $this->input->post('eorg_id'),
                'apply_charge' => $this->input->post('eapply_charge'),
                'created_at' => date('Y-m-d'),
            );
            $this->charge_model->update_opdcharges($data);
            $json_array = array('status' => 'success', 'error' => '', 'message' => 'Record Update Successfully');
        }
        echo json_encode($json_array);
    }

    public function update_ipdcharges()
    {
        $this->form_validation->set_rules('echarge_type', $this->lang->line('charge') . " " . $this->lang->line('type'), 'required');
        $this->form_validation->set_rules('echarge_category', $this->lang->line('charge') . " " . $this->lang->line('category'), 'required');
        $this->form_validation->set_rules('eapply_charge', $this->lang->line('applied') . " " . $this->lang->line('charge'), 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->charge_model->update_ipdcharges($data);
            $json_array = array('status' => 'success', 'error' => '', 'message' => 'Record Saved Successfully');
        }
        echo json_encode($json_array);
    }

    public function getAmbulanceCharges()
    {
        $id = $this->input->post("id");
        $data = $this->charge_model->fetchAmbulanceCharges($id);
    ...
    public function getCharges($id)
    {
        ...
        $result = $this->charge_model->getOpdchargesDetails($id);
        $data['result'] = $result;
        $this->load->view('admin/charges/opdchargesprint', $data);
    }
    public function getIpdCharges($id)
    {
        ...
        $result = $this->charge_model->getIpdchargesDetails($id);
        $data['result'] = $result;

        $this->load->view('admin/charges/ipdchargesprint', $data);
    }
}

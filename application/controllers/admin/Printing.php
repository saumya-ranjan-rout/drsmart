...
class Printing extends Admin_Controller {
  ...
  function appointmentprinting() {
        if (!$this->rbac->hasPrivilege('appointment_print_header_footer', 'can_view')) {
           access_denied();
        }
        $this->session->set_userdata('top_menu', 'setup');
  ...
  function paymentreceiptprinting() {
        if (!$this->rbac->hasPrivilege('ipd_bill_print_header_footer', 'can_view')) {
           access_denied();
        }
        $this->session->set_userdata('top_menu', 'setup');
        $this->session->set_userdata('sub_sidebar_menu', 'admin/printing/paymentreceiptprinting');
        $this->session->set_userdata('sub_menu', 'admin/printing');
        $data["printing_list"] = $this->printing_model->get('', 'payment_receipt');
        $this->load->view('layout/header');
        $this->load->view('admin/printing/paymentreceiptprinting', $data);
        $this->load->view('layout/footer');
    }

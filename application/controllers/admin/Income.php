...
class Income extends Admin_Controller {
    ...
    public function incomeSearch() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            // Redirect with search_type in URL to avoid resubmission
            redirect('admin/income/incomesearch?search_type=' . urlencode($search_type));
        }
...
//   exit;
    ...
    public function incomegroup() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            // Redirect to avoid form resubmission and include search_type and head_id in URL
            redirect('admin/income/incomegroup?search_type=' . urlencode($search_type) . '&head_id=' . urlencode($head_id));
        }
        ...
        // Fetch the income report data based on search type and head_id
        $incomeList = $this->income_model->searchincomegroup($search_type, $head_id);
        $data['incomeList'] = $incomeList;
    
        // Fetch income heads for the dropdown
        $data['headlist'] = $this->incomehead_model->get();

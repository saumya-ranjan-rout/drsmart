...
public function getManuallyTransferingPatient($id) {
    $this->db->select('tm.*');  // Select all columns from both tables
    $this->db->from('transfer_manually tm');  // Use an alias for the transfer_manually table
    $this->db->where('tm.patientid', $id);  // Filter by the patient ID

    $query = $this->db->get();

    if ($query->num_rows() == 1) {
        // Return the row as an associative array
        return $query->row_array();  // Use row_array() to get an associative array
    } else {

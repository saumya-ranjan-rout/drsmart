...
//---------------------------Server side code datatable--------------------------------------
 ...
 public function search_datatable() {
        $current_month_year = date('M/Y');  // e.g., May/2024
        ...
        $this->db->select('pharmacy.*, 
                           ...
                           (SELECT SUM(available_quantity) 
                            ...
                            WHERE pharmacy_id = pharmacy.id 
                             ) as total_qty,(SELECT SUM(available_quantity) 
                            FROM medicine_batch_details 
                            WHERE pharmacy_id = pharmacy.id 
                              AND STR_TO_DATE(CONCAT("01-", medicine_batch_details.expiry_date), "%d-%b/%Y") < STR_TO_DATE(CONCAT("01-", "' . $current_month_year . '"), "%d-%M/%Y")) as expired_qty');
        ...
        if (!empty($_POST['search']['value'])) {  // if there is a search parameter
            $this->db->group_start();
            $counter = true;
            foreach ($this->column_search as $column_key => $column_value) {
                if ($counter) {
                    $this->db->like($column_value, $_POST['search']['value']);
                    $counter = false;
                } else {
                    $this->db->or_like($column_value, $_POST['search']['value']);
                }
            }
            $this->db->group_end();
        }
       ...
       public function getExpiredMedicine($select, $join = array(), $table_name, $search_table, $search_column, $additional_where = array()) {
        $current_month_year = date('M/Y');  // e.g., May/2024
    
        // Create the additional WHERE clause for SQL query
        $expiry_condition = "STR_TO_DATE(CONCAT('01-', medicine_batch_details.expiry_date), '%d-%b/%Y') < STR_TO_DATE(CONCAT('01-', '$current_month_year'), '%d-%M/%Y')";
    
        // Add the expiry condition to the additional where clauses
        array_unshift($additional_where, $expiry_condition);
    ...
    public function updateMedicineBatchDetail($data1) {
        ...
        $this->db->update_batch('medicine_batch_details', $data1, 'id');
        $this->db->last_query();
    }
    ...
    public function delete_pharmacy_bill_detail($delete_arr) {
        foreach ($delete_arr as $key => $value) {
            $id = $value["id"];
            $row = $this->db->select("medicine_category_id, medicine_name, batch_no, quantity")
                            ->where("id", $id)
                            ->get('pharmacy_bill_detail')
                            ->row_array();
    
            // Increment the available_quantity field by the quantity
            $this->db->where(array(
                "medicine_category_id" => $row["medicine_category_id"], 
                "pharmacy_id" => $row["medicine_name"], 
                "batch_no" => $row["batch_no"]
            ));
            $this->db->set('available_quantity', 'available_quantity + ' . (int)$row["quantity"], FALSE);
            $this->db->update('medicine_batch_details');
    
            // Delete the row from pharmacy_bill_detail
            $this->db->where("id", $id)->delete("pharmacy_bill_detail");
        }
    }
   ...
   public function getMaxIdPharmacy()
    {
        $query  = $this->db->select('max(id) as maxid')->get("pharmacy_bill_basic");
        $result = $query->row_array();
        return $result["maxid"];
    }
}

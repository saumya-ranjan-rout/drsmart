...
class Itemissue_model extends CI_Model {
    ...
    public function get($id = null) {
        ...
        $sql = "SELECT item_issue.*,item.name as `item_name`,item.item_category_id,item_category.item_category ,staff.employee_id,staff.name as docname,staff.surname as docsurname,roles.name FROM `item_issue` left JOIN item on item.id=item_issue.item_id left JOIN item_category on item_category.id=item.item_category_id left JOIN staff on staff.id=item_issue.issue_to left JOIN staff_roles on staff_roles.staff_id =staff.id left JOIN roles on roles.id= staff_roles.role_id ORDER BY item_issue.issue_date DESC";

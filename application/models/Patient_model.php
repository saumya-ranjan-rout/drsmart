...
class Patient_model extends CI_Model {
   ...
   public function patientDetails($id) {
        ...
        $this->db->select('patients.*, organisation.organisation_name')
        ->from('patients')
        ->where('patients.id', $id)
        ->join('organisation', 'patients.tpa = organisation.id', 'left'); // Joining organisation table based on tpa field
...
return $query->row_array(); // Return a single row as an associative array
    ...
    public function searchAll($searchterm) {
        $this->db->select('patients.*,death_report.id as did')
                ->from('patients')
                ->join('death_report', 'patients.id = death_report.patient', 'left')
                ->like('patients.patient_name', $searchterm)
                ->or_like('patients.guardian_name', $searchterm)
                ->or_like('patients.patient_type', $searchterm)
                ->or_like('patients.address', $searchterm)
    ...
    public function getpatientDetails($id) {
        // Select the patient details along with the organisation name
        $this->db->select('patients.*, organisation.organisation_name')
                 ->from('patients')
                 ->where('patients.id', $id)
                 ->join('organisation', 'patients.tpa = organisation.id','left'); // Joining organisation table based on tpa field
    ...
    public function searchFullText1($opd_month, $searchterm, $carray = null,$limit=100,$start="") {
       ...
       // $this->db->order_by('opd_details.id', 'desc');
        ...
        //$this->db->group_by(['opd_details.patient_id', 'opd_details.id', 'patients.patient_name', 'patients.patient_unique_id', 'patients.guardian_name', 'patients.gender', 'patients.mobileno', 'patients.is_ipd', 'staff.name', 'staff.surname']);
    ...
    // Subquery to get the latest appointment_date for each patient
    $subquery = "(SELECT patient_id, MAX(appointment_date) AS max_appointment_date
                  FROM opd_details
                  GROUP BY patient_id) AS latest_appointments";

    // Main query
    $this->db->select('opd_details.*,
                       patients.id as pid,
                       ...
                       staff.surname')
             ->from('opd_details')
             ->join($subquery, 'opd_details.patient_id = latest_appointments.patient_id AND opd_details.appointment_date = latest_appointments.max_appointment_date', 'INNER')
             ->join('patients', 'patients.id = opd_details.patient_id', 'LEFT')
             ->join('staff', 'staff.id = opd_details.cons_doctor', 'LEFT')
             ->group_start()
             ->like('patients.patient_name', $searchterm)
             ->or_like('patients.guardian_name', $searchterm)
             ->group_end()
             ->order_by('latest_appointments.max_appointment_date', 'desc')
             ->limit($limit, $start);
    ...
    public function getIpdDetails($id, $ipdid='', $active = 'yes') {
        ...
        $this->db->join('organisation', 'organisation.id = patients.organisation', "left");
         $this->db->join('hospital_department', 'hospital_department.id = ipd_details.hospital_department', "left");
    ...
    public function getOPDetails($id, $opdid = null) {
        ...
        $this->db->select('opd_details.*,opd_details.organisation,patients.old_patient,staff.id as staff_id,staff.name,staff.surname,consult_charges.standard_charge')->from('opd_details');
        $this->db->join('staff', 'staff.id = opd_details.cons_doctor', "inner");
    ...
    function search_ipd_patients($searchterm, $active = 'yes', $discharged = 'no', $patient_id = '',$limit="",$start="") {
        ...
        $this->db->select('patients.*,bed.name as bed_name,bed_group.name as bedgroup_name, floor.name as floor_name,ipd_details.date,ipd_details.id as ipdid,ipd_details.credit_limit as ipdcredit_limit,ipd_details.case_type,ipd_details.ipd_no,staff.name,staff.surname, ipd_details.discharge_status
              ')->from('patients');
    ...
    public function printVisitDetails($patient_id, $visitid) {
        ...
        $query = $this->db->select("patients.*,organisation.organisation_name,opd_details.opd_no,opd_details.amount as apply_charge, opd_details.id as opdid,opd_details.appointment_date,opd_details.symptoms,opd_details.case_type,opd_details.casualty,opd_details.note_remark,staff.name,staff.surname,opd_details.cheque_no,opd_details.cheque_date,opd_details.payment_mode")
    ...
    public function death_patientDetails($id) {
    $this->db->select('patients.*, ipd_details.ipd_no, opd_details.opd_no')
             ->from('patients')
   ...
   public function female_patientDetails($id) {
    ...
    public function getFemalePatientListall() {
        $this->db->select('patients.*')->from('patients');
        $this->db->where('patients.is_active', 'yes');
    ...
    {
        $this->db->where("id", $id)->update("ipd_details", $data);
    }

    public function getTreatmentHistory($id, $ipdid)
    {
        $query = $this->db->select('ipd_details.patient_id, ipd_details.ipd_no, ipd_details.symptoms, patients.id as pid, patients.patient_name, patients.age, patients.guardian_name, patients.guardian_address, patients.admission_date, patients.gender, staff.name as doctorname, staff.surname, bed.name as bed_name,bed.id as bed_id,bed_group.name as bedgroup_name,floor.name as floor_name')
            ->join('patients', 'ipd_details.patient_id = patients.id')
            ->join('staff', 'staff.id = ipd_details.cons_doctor', "inner")
            ...
            ->get('ipd_details');
        return $query->row_array();  // Fetching only one row
    }

    public function add_patient_nursenotes($data)
    {
        if (isset($data["id"])) {

            $this->db->where("id", $data["id"])->update("nurse_notes", $data);
        } else {
    ...
    public function getPatientNurseNote($id)
    {
        // First, get the nurse notes for the specified patient ID
        $this->db->select('nurse_notes.*, staff.name as nursename, staff.surname as nursesurname');
        $this->db->join('staff', 'staff.id = nurse_notes.nurse', 'inner');
        $this->db->where('nurse_notes.patient_id', $id);
        $this->db->order_by('nurse_notes.nurse_note_date', 'desc');
        $nurse_notes = $this->db->get('nurse_notes')->result_array();

        // Initialize an array to hold the nurse notes with their comments
        $nurse_notes_with_comments = [];

        // Loop through each nurse note to fetch comments
        foreach ($nurse_notes as $note) {
            // Fetch comments related to the current nurse note
            $this->db->select('comment_nursenote.*');
            $this->db->where('comment_nursenote.nurse_note_id', $note['id']);
            $comments = $this->db->get('comment_nursenote')->result_array();

            // Append the comments to the current nurse note
            $note['comments'] = $comments;

            // Add the nurse note with its comments to the main array
            $nurse_notes_with_comments[] = $note;
        }

        return $nurse_notes_with_comments;
    }

    public function delete_nursenote($id)
    {

        $this->db->where("id", $id)->delete("nurse_notes");
    }

    public function geteditNursenote($id)
    {

        $query = $this->db->select('nurse_notes.*, staff.name, staff.surname')
            ->join('staff', 'staff.id = nurse_notes.nurse', 'inner')
            ->where('nurse_notes.id', $id)
            ->get('nurse_notes');

        return $query->row_array();
    }

    public function add_comment_nursenote($data)
    {
        if (isset($data["id"])) {

            $this->db->where("id", $data["id"])->update("comment_nursenote", $data);
        } else {

            $this->db->insert("comment_nursenote", $data);
            return $this->db->insert_id();
        }
    }
    ...
    public function delete_comment_nursenote($id)
    {
        ...
        $this->db->where("id", $id)->delete("comment_nursenote");
    }

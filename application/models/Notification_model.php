...
LEFT JOIN read_notification ON send_notification.id = read_notification.notification_id and read_notification.parent_id=" . $this->db->escape($parentid) . " where send_notification.visible_parent='Yes' order by send_notification.publish_date desc");
    ...
    public function expiredMedicineNotification() {
        $data = array();
        
        $select = 'medicine_batch_details.*';
        $table_name = "medicine_batch_details";
        $join = array();  // Keeping the joins empty as per your current requirement
        ...
        $additional_where = array($expiry_condition);
...
public function deleteall() {
        // Attempt to delete all records in 'system_notification' table
        return $this->db->empty_table('system_notification'); // Use empty_table to delete all rows
    }

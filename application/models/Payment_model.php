...
class Payment_model extends CI_Model
{
    ...
    public function opdEditPaymentDetails($id)
    {
        $query = $this->db->select('opd_payment.*,')
            ->where("id", $id)
            ->get("opd_payment");
        return $query->row_array();
    }

    public function updateOPDPayment($data)
    {
        //$this->db->insert("opd_patient_charges", $data);
        $query = $this->db->where('id', $data['id'])
            ->update('opd_payment', $data);
    }

    public function ipdEditPaymentDetails($id)
    {
        $query = $this->db->select('payment.*,')
            ->where("id", $id)
            ->get("payment");
        return $query->row_array();
    }
    ...
    public function opdPrintPaymentDetails($id)
    {
        $query = $this->db->select('opd_payment.*,patients.id as pid,patients.note as pnote, patients.patient_name, patients.patient_unique_id')
            ->join("patients", "patients.id = opd_payment.patient_id")->where("opd_payment.id", $id)
            ->get("opd_payment");
        return $query->row_array();
    }
    public function ipdPrintPaymentDetails($id)
    {
        $query = $this->db->select('payment.*,patients.id as pid,patients.note as pnote,patients.patient_name,patients.patient_unique_id')
            ->join("patients", "patients.id = payment.patient_id")->where("payment.id", $id)
            ->get("payment");
        return $query->row_array();
    }

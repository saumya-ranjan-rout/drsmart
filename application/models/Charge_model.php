...
ORDER BY organisation.id");
    ...
    public function geteditCharges($id)
    {
        $query = $this->db->select('opd_patient_charges.*,charges.charge_type,charges.charge_category,charges.standard_charge,charges.id as cid,')
            ->join('charges', 'opd_patient_charges.charge_id = charges.id', 'inner')
            ->where('opd_patient_charges.id', $id)
            ->get('opd_patient_charges');
        return $query->row_array();
    }

    public function update_opdcharges($data)
    {
        //$this->db->insert("opd_patient_charges", $data);
        $query = $this->db->where('id', $data['id'])
            ->update('opd_patient_charges', $data);
    }

    public function geteditIpdCharges($id)
    {
        $query = $this->db->select('patient_charges.*,charges.charge_type,charges.charge_category,charges.standard_charge,charges.id as cid')
    ...
    public function update_ipdcharges($data)
    {
        //$this->db->insert("opd_patient_charges", $data);
        $query = $this->db->where('id', $data['id'])
    ...
    public function fetchAmbulanceCharges($id)
    {
        $query = $this->db->select('standard_charge')
            ->where('charge_category', $id)
            ->get('charges');
        return $query->row_array();
    }

    public function getAmbulanceCategory()
    {
        $query = $this->db->where("charge_type", "Ambulance")->get("charge_categories");
        return $query->result_array();
    }
    public function getOpdchargesDetails($id)
    {
        $query = $this->db->select('opd_patient_charges.*, patients.patient_name,  patients.patient_unique_id,charges.charge_type, charges.charge_category, charges.standard_charge, organisations_charges.id as oid, organisations_charges.org_charge')
    ...
    public function getIpdchargesDetails($id)
    {
        $query = $this->db->select('patient_charges.*, patients.patient_name,  patients.patient_unique_id,charges.charge_type, charges.charge_category, charges.standard_charge, organisations_charges.id as oid, organisations_charges.org_charge')

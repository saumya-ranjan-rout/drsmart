...
class Setting_model extends CI_Model {
  ...
  public function getPrefixSetting($column_name)
{
    // Fetch the prefix from the prefix_setting table based on the column name
    $this->db->select($column_name);
    $this->db->from('prefix_setting');
    $query = $this->db->get();
    
    // Check if the query returns a result
    if ($query->num_rows() > 0) {
        return $query->row()->$column_name; // Return the prefix for the column
    } else {
        return ""; // If not found, return an empty string or default value
    }
}

...
class Operationtheatre_model extends CI_Model
{
    ...
    {
        if (isset($operation_detail['id'])) {
            $this->db->where('id', $operation_detail['id']);
            $this->db->update('operation_theatre', $operation_detail);
        } else {
    ...
    public function getOperationPatientDetails($otid)
    {
        $this->db->select('operation_theatre.*,patients.id as pid,patients.patient_unique_id,patients.patient_name,patients.admission_date,patients.gender,patients.age,patients.month,patients.patient_type,patients.guardian_name,patients.mobileno,patients.organisation,patients.blood_group,patients.known_allergies,patients.note,patients.marital_status,patients.guardian_address,patients.is_active,patients.email,patients.address,patients.dob,staff.name,staff.surname,charges.id as cid,charges.charge_category,charges.code,charges.standard_charge,charges.description,charge_categories.id as charge_categories_id,charge_categories.name as charge_categories_name')->from('operation_theatre');
        $this->db->join('patients', 'operation_theatre.patient_id=patients.id', "inner");
        $this->db->join('staff', 'staff.id = operation_theatre.consultant_doctor', "inner");
        $this->db->join('charges', 'operation_theatre.code = charges.id');
        $this->db->join('charge_categories', 'operation_theatre.charge_id = charge_categories.id');
        $this->db->where('operation_theatre.id', $otid);
        $query = $this->db->get();
        return $query->row_array();
    }
    public function getOperationDetails($pid,$id)
    {
        $this->db->select('operation_theatre.*,patients.id as pid,patients.patient_unique_id,patients.patient_name,patients.admission_date,patients.gender,patients.age,patients.month,patients.patient_type,patients.guardian_name,patients.mobileno,patients.guardian_address,patients.is_active,staff.name,staff.surname,charges.id as cid,charges.charge_category,charges.code,charges.standard_charge,charges.description,organisation.organisation_name')->from('operation_theatre');
        $this->db->join('patients', 'operation_theatre.patient_id=patients.id', "inner");
        $this->db->join('staff', 'staff.id = operation_theatre.consultant_doctor', "inner");
        ...
        $this->db->where('patients.is_active', 'yes');
        $this->db->where('operation_theatre.id', $id);
       // $this->db->or_where('patients.id', $pid);
        $query = $this->db->get();
        return $query->row_array();
    }
    ...
    public function addOtCategory($data)
    {
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $this->db->update('operation_category', $data);
        } else {
            $this->db->insert('operation_category', $data);
            return $this->db->insert_id();
        }
    }

    public function getOtCategory($id = null)
    {
        if (!empty($id)) {
            $query = $this->db->where("id", $id)->get('operation_category');
            return $query->row_array();
        } else {
    ...
    public function getotCategoryName($id = null)
    {
        if (!empty($id)) {
            $query = $this->db->where("id", $id)->get('operation_category');
            return $query->row_array();
    ...
    public function deleteotcategory($id)
    {
    ...
    {
        $query = $this->db->select('operation_category.id,operation_category.category_name')->get('operation_category');
        return $query->result_array();
    }

    public function addOperationList($data)
    {
        if (isset($data['id'])) {
            $this->db->where('id', $data['id']);
            $this->db->update('operation_list', $data);
        } else {
            $this->db->insert('operation_list', $data);
            return $this->db->insert_id();
        }
    }

    public function getOtList($id = null)
    {
        if (!empty($id)) {
            $query = $this->db->where("id", $id)->get('operation_list');
            return $query->row_array();
        } else {
            $query = $this->db->get("operation_list");
            return $query->result_array();
        }
    }

    public function getotListName($id = null)
    {
        if (!empty($id)) {
            $query = $this->db->where("id", $id)->get('operation_list');
            return $query->row_array();
    ...
    public function deleteotlist($id)
    {
        $query = $this->db->where('id', $id)
